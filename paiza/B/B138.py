...
#これでドーナツを判定したいんよ 3*3で一個
answer = 0
for i in range(1,H-1):
    for j in range(1,W-1):
        if s[i][j] == '.' and s[i-1][j-1] == '#' and s[i-1][j] == '#' and s[i-1][j+1] == '#' and s[i][j-1] == '#' and s[i][j+1] == '#' and s[i+1][j-1] == '#' and s[i+1][j] == '#' and s[i+1][j+1] == '#':
            answer += 1
print(answer)
...
#ドーナツをこう定義してあげる
for i in range(H-2):#端っこから一個文は意味ない
    for j in range(W-2):
        ok = True
        for di in range(3):
            for dj in range(3):
                if s[i + di][j + dj] != dontu[di][dj]:
                    ok = False
