"""
paiza.B.B045 の Docstring
学校低学年向けの足し算と引き算の計算ドリルをつくりましょう。計算ドリルとして問題に重複があると計算力の訓練にならないため、足し算、引き算それぞれで重複が無いようにつくらなければなりません。必要な足し算と引き算の問題の数が与えられるので、それに従ってドリルを作成してください。
...
p_M+N
期待する出力は M + N 行からなります。
出力の i 行目 (1 ≦ i ≦ M + N) に問題文の条件を満たす i 番目の問題を表す文字列 p_i を出力してください。
p_i は整数 a, b に対して足し算であれば "a + b ="、引き算であれば "a - b =" という形式です (空白部分は半角スペース 1 つ)。
...
while count < M:
    ...
    if a + b <= 99 and (a, b) not in used_add:
        print(f"{a} + {b} =")
        used_add.add((a, b))
        count += 1
