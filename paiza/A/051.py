...
for j in range(W):
    dp[H-1][j] = S[H-1][j] 
for i in range(H-2,-1,-1):#-1は含まれない(0)までのループで終了するため
    for j in range(W):
        best = dp[i+1][j]#とりま真下に行く
        if j > 0:
            best = max(best,dp[i+1][j-1]) #左の列見る
        if j < W - 1:
            best = max(best, dp[i+1][j+1]) #右の列見る
