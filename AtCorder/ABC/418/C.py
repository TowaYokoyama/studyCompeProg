""""
問題文
ポーカーテーブルの上に N 種類のフレーバーのティーバッグが置かれています。フレーバーには 1 から N までの番号が付けられており、フレーバー i (1≤i≤N) のティーバッグは A 
i
​	
  個あります。

あなたはこれらのティーバッグを使ったゲームに参加します。ゲームには 1 以上 A 
1
​	
 +⋯+A 
N
​	
  以下の難易度が設定されており，難易度 b のゲームは以下の流れで行われます。

あなたは整数 x を宣言する。このとき、b≤x≤A 
1
​	
 +⋯+A 
N
...
j
​	
  のゲームに勝利するためにはじめに宣言する必要がある整数 x の最小値を答えよ。勝利が不可能であれば、代わりに −1 と答えよ。
制約
1≤N≤3×10 
5
...
j 行目 (1≤j≤Q) には、j 番目のクエリに対する答えを出力せよ。
