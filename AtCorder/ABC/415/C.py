"""
AtCorder.ABC.415.C の Docstring
問題文
N 種類の薬品 1,2,…,N があります。あなたの目標はこれらが全て混ざった状態にすることです。
0, 1 からなる長さ 2 
N
 −1 の文字列 S が与えられます。この文字列は次の情報を表します。

まず、 1 種類以上の薬品が混ざった状態 i ( 1≤i≤2 
N
 −1 ) を次のように定義する。
i を 2 進法で表記した際に下から k ( 1≤k≤N ) 桁目が 1 である、またその時に限り、薬品 k が含まれている。
例えば、 13 を 2 進法で表記すると 1101 
(2)
​	
  となるため、状態 13 は薬品 1,3,4 が混ざった状態を表現します。
