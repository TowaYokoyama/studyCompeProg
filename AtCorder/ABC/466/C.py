...
数直線上に、点 1, 2, …, N が この順に 左から右に並んでいます。

最初、あなたには整数 N のみが与えられます。

その後、あなたはジャッジに以下の質問を 2N 回まで行うことができます。

1≤i<j≤N を満たす整数 i,j を選び、点 i と点 j の距離が 1 以下であるか質問する。
距離が 1 以下の 2 点組の個数、すなわち 1≤i<j≤N をみたす整数の組 (i,j) であって、点 i と点 j の距離が 1 以下であるようなものの個数を出力してください。
...
質問は、以下の形式で標準出力に出力してください。 ここで、i,j は 1≤i<j≤N を満たす整数である必要があります。
